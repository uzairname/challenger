...
async def update_match_outcome(ctx:tanjun.abc.Context, match, new_outcome, staff_declared=None):
    ...
    if match["p1_is_ranked"]:
        ...
        p1_matches = DB.get_matches(user_id=match["player_1"], number=NUM_UNRANKED_MATCHES, from_first=True)
    ...
    if match["p2_is_ranked"]:
        ...
        p2_matches = DB.get_matches(user_id=match["player_2"], number=NUM_UNRANKED_MATCHES, from_first=True)
